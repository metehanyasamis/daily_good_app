...
class _RatingFormCardState extends ConsumerState<RatingFormCard> {
  ...
  bool _isSubmittedSuccess = false; // Sadece başarılı 200 OK durumunda true olacak
  ...
  Widget build(BuildContext context) {
    // 🔒 KİLİTLEME MANTIĞI:
    // 1. Zaten eski bir yorum varsa (Backend'den dolu geldiyse)
    // 2. VEYA yeni işlem BAŞARIYLA (200 OK) bittiyse.
    final bool isReadOnly = widget.existingReviewId != null || _isSubmittedSuccess;
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 6),
              if (isReadOnly)
                const Icon(Icons.check_circle, size: 18, color: AppColors.primaryDarkGreen)
            ],
          ),
          const SizedBox(height: 8),
          ...
          TextField(
            controller: _commentController,
            maxLines: 3,
            enabled: !isReadOnly, // Kilitliyse yazamaz
            style: TextStyle(color: isReadOnly ? Colors.black54 : Colors.black),
            decoration: InputDecoration(
              hintText: isReadOnly ? '' : 'Görüşlerin bizim için çok değerli 💚\n(İsteğe bağlı)',
              hintStyle: const TextStyle(color: Colors.black45, fontSize: 14),
  ...
  // ✅ Sadece işlem başarılıysa veya eski yorumsa görünür
  Widget _buildReadOnlyFooter() {
    return Column(
      children: [
        const Center(
  ...
  Future<void> _handleSubmit() async {
    ...
    // 2. Backend İsteği (Await ile sonucu bekle)
    final success = await controller.submitReview(
      storeId: widget.storeId,
      productId: widget.productId,
      existingReviewId: widget.existingReviewId,
    ...
    if (success) {
      // ✅ Sadece Backend 200 OK dönerse burası çalışır
      debugPrint("🎉 UI Update: Success Dialog shown");
      HapticFeedback.mediumImpact();

      setState(() {
        _isSubmittedSuccess = true; // Ekranı kilitle ve yeşil mesajı göster
      });

      _showSuccessDialog(context);

    } else {
      // ❌ Hata durumunda (400, 500, Zaten yapılmış vs.)
      // Ekranı KİLİTLEMİYORUZ (_isSubmittedSuccess false kalıyor)
      // Kullanıcı sadece kırmızı hatayı görüyor.
      ...
      if (errorMessage.contains("zaten") || errorMessage.contains("mevcut")) {
        Toasts.error(context, 'Bu siparişi zaten değerlendirdiniz.');
      } else {
        Toasts.error(context, errorMessage);
      }
            ...
            child: Material(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    "Geri bildirimin başarıyla iletildi. Teşekkür ederiz 💚",
                    textAlign: TextAlign.center,
                  ),
                  const SizedBox(height: 24),
                  SizedBox(
        ...
        children: [
          ...
          Wrap(
            ...
            children: List.generate(5, (index) {
              ...
              final bool isFilled = starValue <= _ratings[label]!;
              ...
              return GestureDetector(
                onTap: isReadOnly ? null : () {
                  ...
                  setState(() {
                    // 🔥 YENİ MANTIK BURADA:
                    // Eğer tıklanan yıldız zaten seçili olan değerse -> Sıfırla (0)
                    if (_ratings[label] == starValue) {
                      _ratings[label] = 0;
                    } else {
                      // Değilse -> Yeni değeri seç
                      _ratings[label] = starValue;
                    }
                  });
                },
                child: Icon(
                  ...
                  size: 24,
                  color: isFilled
                      ? (isReadOnly ? AppColors.primaryDarkGreen.withValues(alpha: 0.5) : AppColors.primaryDarkGreen)
                      : Colors.grey.shade300,
                ),
              );
            }),
