...
class _ContactScreenState extends ConsumerState<ContactScreen> {
  ...
  Future<void> _submit() async {
    ...
    if (selectedSubjectKey == null) {
      _toast("Lütfen bir konu başlığı seçin", isError: true);
      return;
    }

    if (selectedSubjectKey == "other" && messageText.isEmpty) {
      _toast("Konu 'Diğer' olduğunda mesaj yazmanız zorunludur.", isError: true);
      return;
    }
    ...
    try {
      ...
      HapticFeedback.mediumImpact(); // 🎯 Başarılı gönderim hissi
      context.go('/contact-success');
    } catch (e) {
      _toast("Hata: ${e.toString().replaceAll("Exception: ", "")}", isError: true);
    }
  }
  ...
  // 🎯 REFACTOR EDİLEN TOAST METODU
  void _toast(String msg, {bool isError = false}) {
    if (!mounted) return;
    if (isError) {
      HapticFeedback.vibrate(); // 🛡️ Hata titreşimi
      Toasts.error(context, msg);
    } else {
      HapticFeedback.selectionClick(); // 🛡️ Bilgi tıkı
...
class _PickerTile extends StatelessWidget {
  ...
  const _PickerTile(this.source, this.icon, this.title);
